...
let indent_to_scope (counted_lst: (int * token list) list) =
    ...
    let rec flush_indent acc = 
        if Stack.length st > 1 then (Stack.drop st; flush_indent (DEDENT :: acc))
        else
            acc
    in
    let rec go lst acc = 
...
let prepare_lexbuf lexbuf = get_token_list lexbuf |> split_by_line |> get_indent_width |> indent_to_scope |> List.flatten |> create_lexbuf
